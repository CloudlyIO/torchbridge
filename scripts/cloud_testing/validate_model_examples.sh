...
run_example "deepseek_inference" \
    "examples/models/llm/deepseek_cross_backend.py" \
...
run_example "qwen3_inference" \
    "examples/models/llm/qwen3_cross_backend.py" \
    --quantization "$LLM_QUANT" \
    --max-new-tokens 128 \
...
run_example "gemma3_inference" \
    "examples/models/llm/gemma3_cross_backend.py" \
    --model "$GEMMA_MODEL" \
...
if [ "$LLAMA4_QUANT" != "skip" ]; then
    ...
    run_example "llama4_inference" \
        "examples/models/llm/llama4_cross_backend.py" \
