...
class TestLargeBatchInference:
    ...
    def test_minilm_batch_scaling(self, minilm_model, batch_size):
    ...
    @pytest.mark.parametrize("batch_size", BATCH_SIZES)
    def test_dinov2_batch_scaling(self, dinov2_model, batch_size):
        """Verify DINOv2 handles increasing batch sizes without error."""
        images = torch.randn(batch_size, 3, 224, 224)
        with torch.no_grad():
            output = dinov2_model(images)
        assert output.last_hidden_state.shape[0] == batch_size
    ...
    def test_gpu_batch_scaling(self, minilm_model, batch_size):
