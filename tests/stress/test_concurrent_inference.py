...
class TestConcurrentInference:
    def test_two_models_same_device_cpu(self, minilm_model, dinov2_model):
        """Two different models coexist and produce correct output on CPU."""
        text_model, tokenizer = minilm_model
        vision_model = dinov2_model
    ...
    def test_interleaved_inference(self, minilm_model, dinov2_model):
    ...
    @pytest.mark.gpu
    def test_two_models_same_gpu(self, minilm_model, dinov2_model):
        """Two models on same GPU, both produce valid output."""
        device = torch.device("cuda")
        text_model, tokenizer = minilm_model
        text_gpu = text_model.to(device)
        vision_gpu = dinov2_model.to(device)
