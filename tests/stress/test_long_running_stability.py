...
class TestLongRunningStability:
    ...
    def test_minilm_1000_iterations_no_drift(self, minilm_model):
    ...
    def test_dinov2_1000_iterations_no_drift(self, dinov2_model):
        """1000 forward passes on vision model produce identical output."""
        image = torch.randn(1, 3, 224, 224)

        with torch.no_grad():
            baseline = dinov2_model(image).last_hidden_state.clone()

        for i in range(self.ITERATIONS):
            with torch.no_grad():
                output = dinov2_model(image).last_hidden_state
            if (i + 1) % self.CHECK_INTERVAL == 0:
                max_diff = torch.abs(baseline - output).max().item()
                assert max_diff < 1e-6, (
    ...
    def test_1000_iterations_no_memory_leak(self, minilm_model, memory_tracker):
    ...
    @pytest.mark.gpu
    def test_gpu_1000_iterations_stable(self, minilm_model):
