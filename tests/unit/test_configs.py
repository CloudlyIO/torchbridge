...
class TestDataConfig:
    ...
    def create_tensors(
        ...
        q = torch.randn(
            self.batch_size, self.num_heads, self.seq_len, self.head_dim, device=device
        )
        k = torch.randn(
            self.batch_size, self.num_heads, self.seq_len, self.head_dim, device=device
        )
...
def print_config_summary():
    ...
    print(
        f"{'Config':<12} {'Dims (B×H×S×D)':<20} {'Memory':<10} {'Target':<10} {'Use Case'}"
    )
    print("-" * 80)

    for name, config in TEST_CONFIGS.items():
        dims = (
            f"{config.batch_size}×{config.num_heads}×{config.seq_len}×{config.head_dim}"
        )
        memory = f"{config.total_memory_mb:.1f}MB"

        print(
            f"{name:<12} {dims:<20} {memory:<10} {config.target_time:<10} {config.use_case}"
        )
