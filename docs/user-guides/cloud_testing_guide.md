...
kind: ConfigMap
metadata:
  name: pytorch-test-config
  namespace: pytorch-optimization
data:
  setup.sh: |
    #!/bin/bash
    cd /workspace
...
oc apply -f pytorch-gpu-project.yaml
oc apply -f gpu-test-pod.yaml

# Monitor pod startup
oc logs -f pytorch-gpu-test -n pytorch-optimization

# Execute tests in pod
oc exec -it pytorch-gpu-test -n pytorch-optimization -- bash
cd /workspace
python3 -c "import torch; print(f'CUDA: {torch.cuda.is_available()}')"
...
def benchmark_attention(batch_size=32, seq_len=512, embed_dim=768):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            pass  # Add your flash attention code
        flash_time = time.perf_counter() - start_time
    except ImportError:
        flash_time = "Not available"
...
def memory_efficient_attention():
    ...
    # Large input that might cause OOM
    batch_size, seq_len, embed_dim = 16, 2048, 1024

    x = torch.randn(batch_size, seq_len, embed_dim, device=device, requires_grad=True)

    # Standard attention (may fail with OOM)
    try:
        attn = torch.nn.MultiheadAttention(embed_dim, 16).to(device)
        output, _ = attn(x, x, x)
        loss = output.sum()
        loss.backward()
        print("✅ Standard attention: Success")
        return True
    ...
    try:
        ...
        attn_checkpoint = lambda x: checkpoint.checkpoint(attn, x, x, x)[0]
        output = attn_checkpoint(x)
        loss = output.sum()
        loss.backward()
        print("✅ Checkpointed attention: Success")
        return True
    except torch.cuda.OutOfMemoryError:
        print("❌ Checkpointed attention: OOM")
        return False
...
# OCP
oc get pods -n pytorch-optimization
oc delete pod pytorch-gpu-test -n pytorch-optimization
oc delete project pytorch-optimization
```
