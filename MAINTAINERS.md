# Maintainers

This file lists the maintainers of the TorchBridge project.

| GitHub | Role |
|--------|------|
| [@shahirahman](https://github.com/shahirahman) | Lead maintainer |
| [@ishtiak](https://github.com/ishtiak) | Maintainer |
| [@pangkajpaulcl](https://github.com/pangkajpaulcl) | Maintainer |
| [@mahdi-cloudly](https://github.com/mahdi-cloudly) | Maintainer |

## Responsibilities

Maintainers are expected to:
- Review and merge pull requests
- Triage issues and bug reports
- Ensure CI stays green
- Uphold the [contribution guidelines](CONTRIBUTING.md) and code quality standards

## Contact

Reach the team at [dev@torchbridge.ml](mailto:dev@torchbridge.ml) or open an issue on GitHub.
