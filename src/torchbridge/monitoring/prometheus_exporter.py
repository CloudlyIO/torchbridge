"""
Prometheus Metrics Exporter for TorchBridge

This module provides Prometheus-compatible metrics export for monitoring
TorchBridge model inference in production environments.
...
class MetricsExporter:
    ...
    def get_metrics_text(self) -> str:
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            lines = [
                "# HELP torchbridge_inference_requests_total Total requests",
                "# TYPE torchbridge_inference_requests_total counter",
                f"torchbridge_inference_requests_total {metrics.total_requests}",
                "# HELP torchbridge_inference_latency_ms Average latency",
                "# TYPE torchbridge_inference_latency_ms gauge",
                f"torchbridge_inference_latency_ms {metrics.average_latency_ms:.2f}",
            ]
            return "\n".join(lines)
