...
class CUDADeviceError(DeviceError):
    ...
    def __init__(
        self, device_id: int = 0, operation: str = "", error_message: str = ""
    ):
...
class InvalidComputeCapabilityError(NVIDIABackendError):
    ...
    def __init__(self, compute_capability: str, required: str):
        message = (
            f"Invalid compute capability {compute_capability}, required {required}"
        )
        super().__init__(
            message, {"compute_capability": compute_capability, "required": required}
        )
...
class ModelOptimizationError(BaseModelOptimizationError):
    ...
    def __init__(
        self, model_name: str = "", optimization_type: str = "", error_message: str = ""
    ):
