"""
TorchBridge Unified Optimizations Module
...
for attr_name in dir(current_module):
    if not attr_name.startswith('_') and attr_name not in ['warnings', 'sys']:
        try:
            ...
            if hasattr(attr, '__module__') and ('torchbridge.optimizations' in str(getattr(attr, '__module__', '')) or
                                                'torchbridge.optimization_patterns' in str(getattr(attr, '__module__', '')) or
...
def _deprecation_warning(old_path: str):
    warnings.warn(
        ...
        f"Please use 'from torchbridge.optimizations import ...' instead.",
        DeprecationWarning,
        stacklevel=3
    )
...
class _LegacyOptimizationImportHelper:
    ...
    def __getattr__(self, name):
        _deprecation_warning('torchbridge.optimization_patterns')
        ...
        try:
            ...
            raise AttributeError(f"module 'torchbridge.optimization_patterns' has no attribute '{name}'") from None

class _LegacyNextGenImportHelper:
    """Helper for backward compatibility with old import paths"""
    def __getattr__(self, name):
        _deprecation_warning('torchbridge.next_gen_optimizations')
        # Import the actual module to avoid recursion
        try:
