...
class FusedLinearGELU(nn.Module):
    ...
    def _check_cuda_kernel_availability(self) -> bool:
        ...
        try:
            ...
            return hasattr(torchbridge_cuda, 'fused_linear_gelu')
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self._cuda_kernel_available and x_2d.is_cuda:
            try:
                import torchbridge_cuda
                output = torchbridge_cuda.fused_linear_gelu(
...
class FusedLinearSiLU(nn.Module):
    ...
    def _check_cuda_kernel_availability(self) -> bool:
        ...
        try:
            import torchbridge_cuda
            return hasattr(torchbridge_cuda, 'fused_linear_silu')
        except (ImportError, AttributeError):
            return False
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self._cuda_kernel_available and x_2d.is_cuda:
            try:
                import torchbridge_cuda
                output = torchbridge_cuda.fused_linear_silu(
                    x_2d, self.weight, self.bias
                )
            except Exception as e:
                warnings.warn(f"Fused kernel failed: {e}. Using fallback.", stacklevel=2)
