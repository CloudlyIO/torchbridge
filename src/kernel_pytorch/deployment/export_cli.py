"""
Export CLI for KernelPyTorch (v0.4.25)

Command-line interface for exporting PyTorch models to various formats.

Usage:
    python -m kernel_pytorch.deployment.export_cli export model.pt \
        --format onnx \
        --output model.onnx \
        --opset 14

    python -m kernel_pytorch.deployment.export_cli validate model.pt \
        --sample-input "(1, 512)" \
        --max-latency 50
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="KernelPyTorch Model Export CLI",
...
Examples:
  ...
  Show model info:
    python -m kernel_pytorch.deployment.export_cli info model.pt --verbose
        """
    )
